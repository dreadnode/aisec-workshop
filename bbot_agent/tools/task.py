...
async def finish_task(success: bool, markdown_summary: str) -> rg.Stop:  # noqa: FBT001
    """
    ...
    *   **Comprehensive Summary**: The `summary` should be a complete and detailed markdown-formatted report of everything you did, including steps taken, tools used, and the final outcome. This is your final report to the user.
    ...
    dn.log_output("task_summary", dn.Markdown(markdown_summary), to="run")

    if success:
        dn.tag("success", to="run")

    return rg.Stop(f"Task completed with success={success}")
